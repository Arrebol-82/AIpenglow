...
onMounted(async () => {
  ...
  const startScale = Math.min(
    1.8,
    Math.max(1.15, 1 + (viewportW / 1440) * 0.3),
  );
  ...
  ctx = gsap.context(() => {
    ...
    // Phase 1: film scales from startScale to 1 (0 → 1, full timeline)
    // Phase 2: inspiration scales from 1 to 0.94 (0.8 → 1)
    ...
    tl.to(inspirationWrapperRef.value, { scale: 0.94, ease: "none" }, 0.8);
  }, outerRef.value);
});
