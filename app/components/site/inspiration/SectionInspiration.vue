...
import {
  POLL_DELAY_ACTIVE,
  POLL_DELAY_IDLE,
  RECORD_SPIN_TARGET_VELOCITY,
  RECORD_SPIN_EASING_ACTIVE,
  RECORD_SPIN_EASING_IDLE,
  RECORD_SPIN_VELOCITY_THRESHOLD,
  MUSIC_TITLE_GAP,
  MUSIC_TITLE_MIN_DURATION,
  MUSIC_TITLE_SPEED_DIVISOR,
...
const updateMusicTitleOverflow = () => {
  ...
  musicTitleScrollDuration.value = Math.max(
    MUSIC_TITLE_MIN_DURATION,
    distance / MUSIC_TITLE_SPEED_DIVISOR,
  );
};
...
const stepRecordSpin = (timestamp: number) => {
  ...
  const targetVelocity = activeTrack.value ? RECORD_SPIN_TARGET_VELOCITY : 0;
  const easing = activeTrack.value
    ? RECORD_SPIN_EASING_ACTIVE
    : RECORD_SPIN_EASING_IDLE;
  ...
  if (
    !activeTrack.value &&
    Math.abs(recordSpinVelocity) < RECORD_SPIN_VELOCITY_THRESHOLD
  ) {
